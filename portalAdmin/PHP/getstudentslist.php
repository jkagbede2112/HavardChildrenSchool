...
if ($searchParameter === "secondary") {
    $searchCriteria = validate('searchCriteria');
    echo "$searchCriteria";
    $query = mysqli_query($wc, "select * from schstudent where Firstname like '%$searchCriteria%' or Surname like '%$searchCriteria%' order by ClassID LIMIT 40");
    if (mysqli_num_rows($query) < 1) {
        echo "No students";
    } else {
        while ($rez1 = mysqli_fetch_array($query)) {
            $sID = $rez1['StudentID'];
            $links = mysqli_query($wc, "select * from linkages where StudentID ='$sID'");
            $gtr = mysqli_num_rows($links);
            $classid = $rez1['ClassID'];
            $classname = getclassnamehcs($classid);

            echo "<tr><td>" . $rez1['schoolid'] . "</td><td><b>" . strtoupper($rez1['Surname']) . "</b> " . $rez1['Firstname'] . "</td><td>" . $classname . "</td><td>" . $rez1['dateofbirth'] . "</td><td>" . $gtr . "</tr>";
        }
    }
}
