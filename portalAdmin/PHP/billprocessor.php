...
if ($action === "promotestat"){
    echo "Promote good";
    //action:"promotestat", stdid:stdid, session:session, comment:comment
    $stdid = validate("stdid");
    $session = validate("session");
    $comment = validate("comment");
    $qa = "insert into promotedtable (studentid, comment, session) values ('$stdid','$comment','$session')";
    $zs = mysqli_query($w,$qa);
    if ($zs){
        echo "Saved";
    }else{
        $kt = "update promotedtable set comment ='$comment' where studentid='$stdid' and session='$session'";
        $jt = mysqli_query($w,$kt);
        if ($jt){
            echo "Updated";
        }
    }
...
if ($action === "deleteasset"){
    $assid = validate("assetid");
    $qa = "delete from assetitems where id='$assid'";
...
//deleteassetcat
if ($action === "deleteassetcat"){
    $assid = validate("assetid");
    $qa = "delete from assetcategory where id='$assid'";
    $xz = mysqli_query($w, $qa);
    if ($xz){
        echo "Asset category removed";
    }else{
    }
}
...
function getrepcut($classname, $session){
    global $w;
    $de = $classname;
    $a = "select count(*) as gt from paycatbreakdown where description like '%$de%'";
    $b = mysqli_query($w, $a );
...
if ($action === "createbill") {
    ...
    $paybreakdown = $classname;
    $rep = getrepcut($classname, $session);
    $g = "select * from paycatbreakdown where description like '%$paybreakdown%'";
    //echo $g;
    $ht = mysqli_query($w, $g);
    echo $rep ."<br>";
...
if ($action === "getbillinvoice") {
    ...
    while ($gq = mysqli_fetch_array($it)) {
        ...
        $itg .= "<tr style='font-size:13px'><td class='pdfive'>$count</td><td class='pdfive'>$billname</td><td class='pdfive'>₦ $amount</td><td class='pdfive'>₦ $paymentmade</td><td class='pdfive'>₦ $amtleft </td><td class='pdfive'>₦ <input type='text' size='4' id='$hgt' onblur='makepay(\"$hgt\", \"$billitem\", \"$session\", \"$term\", \"$stdname\")'>"
                . "<!--<input type='button' onclick='' value='Pay'>-->&nbsp; <i class='fa fa-times ptr' title='Delete item' onclick='actinvdel(\"#billinv$billitem\")'></i></td><td><input id='billinv$billitem' style='display:none' type='button' class='btn btn-warning' value='Delete' onclick='deleteinvitem($billitem)'></td></tr>";
        $count++;
    }
...
$hdf = $totalamount;
    ...
    $amtpaid1 = getamtpaid($studentid, $term, $session);
	$amtpaid = number_format($amtpaid1, 2);
    $amtleft = $hdf - ($amtpaid1 + $discount);
...
<tr style='font-size:15px; font-weight:bold'><td></td><td style='padding:5px'>Discount</td><td style='padding:5px'>₦ $discount1</td><td></td></tr>
