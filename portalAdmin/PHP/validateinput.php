...
error_reporting(0);
//$resulttype,
function getgradelevelproprietresscommentforgrade($resulttype, $stdid, $term, $session) {
    $stdclass = getstclassidfromschoolid($stdid);
    $totalscorespossible = gettotalpossiblescores($stdclass);
    $totalscores = gettotalscoresforgrade($resulttype, $stdid, $term, $session);
...
function gettotalscoresforgrade($resulttype, $stdid, $term, $session) {
    global $w;
...
function getgradelevelproprietresscomment($resulttype,$stdid, $term, $session) {
    if ($resulttype === "midterm"){
        $stdclass = getstclassidfromschoolid($stdid);
    $totalscorespossible = gettotalpossiblescoresgperstdz($stdid, $term, $session, $stdclass, $resulttype);
    $totalscores = gettotalscoresgz($stdid, $term, $session, $resulttype);
    //echo $studentid . " ---- ";

    $score = number_format((($totalscores / $totalscorespossible) * 100),2);
        //return $percentage;
...
function gettotalpossiblescoresgperstdz($stdid, $term, $session, $stdclass, $resulttype) {
    //return "$resulttype";
    global $w;
...
function gettotalscoresgz($stdid, $term, $session, $resulttype) {
    global $w;
    if ($resulttype === "midterm") {
...
function getteacheridfromstdid($stdid) {
    global $w;
    //getclassid
...
function getclassnamehcs($classid) {
    global $wc;
