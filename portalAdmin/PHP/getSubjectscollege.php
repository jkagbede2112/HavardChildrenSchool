...
if ($searchKind === "pullsubs") {
    ...
    $g = "select * from subjects_college where SubjectCategoryid = '$classid' order by SubjectName asc";
    $y = mysqli_query($w, $g);
...
if ($searchKind === "putresult") {
    ...
    if ($schooltype === "Playgroup") {
        $table = "playnurresult";
        $o = "select distinct(Subjectgroup) from subjects_college where SubjectCategoryid='$classid' order by Subjectgroup ASC";
        $aq = mysqli_query($w, $o);
        ...
        while ($hq = mysqli_fetch_array($aq)) {
            ...
            $s = "select * from subjects_college where Subjectgroup='$subjectgroupid'";
            $gt = mysqli_query($w, $s);
            $cnt = 1;
...
if ($searchKind === "filter") {
    $values = $_POST['value'];
    $fa = mysqli_query($w, "select * from subjects_college where SubjectCategoryid ='$values' order by SubjectName ASC");
    if (mysqli_num_rows($fa) < 1) {
        ...
        while ($ad = mysqli_fetch_array($fa)) {
            ...
            $ada = $ad["sn"];
            $ddaa = mysqli_query($w, "select * from subjects_college where SubjectID='$subjectID'");
...
if ($searchKind === "all") {
    $fa = mysqli_query($w, "select * from subjects_college order by SubjectCategoryid ASC");
    if (mysqli_num_rows($fa) < 1) {
        echo "No Records found";
    } else {
        $counter = 1;
        while ($ad = mysqli_fetch_array($fa)) {
            $subjectID = $ad['SubjectID'];
            $ada = $ad["sn"];
            $ju = "select * from subjects_college where SubjectID='$subjectID'";
...
if ($searchKind === "search") {
    $value = $_POST['value'];
    $fa = mysqli_query($w, "select * from subjects_college where SubjectName like '%$value%' order by SubjectCategoryid ASC");
    if (mysqli_num_rows($fa) < 1) {
        echo "No Records found";
    } else {
        $counter = 1;
